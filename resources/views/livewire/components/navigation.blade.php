<header class="w-full bg-themeblack text-white relative" x-data="{ show: false }">
        ...
        <div class="flex items-center justify-between gap-4 w-full">
            ...
            <div class="flex items-center gap-4">
                ...
                <svg width="22" height="27" viewBox="0 0 22 27" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <path
                        d="M11 14.75C13.0711 14.75 14.75 13.0711 14.75 11C14.75 8.92893 13.0711 7.25 11 7.25C8.92893 7.25 7.25 8.92893 7.25 11C7.25 13.0711 8.92893 14.75 11 14.75Z"
                        stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                    <path
                        d="M11 1C8.34784 1 5.8043 2.05357 3.92893 3.92893C2.05357 5.8043 1 8.34784 1 11C1 13.365 1.5025 14.9125 2.875 16.625L11 26L19.125 16.625C20.4975 14.9125 21 13.365 21 11C21 8.34784 19.9464 5.8043 18.0711 3.92893C16.1957 2.05357 13.6522 1 11 1Z"
                        stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
                </svg>
                ...
                <div class="flex flex-col">
                    <span class="text-sans text-[#C5C6CC] lg:text-sm xl:text-lg font-semibold">Deliver to {{
                        $user->first_name ?? 'Guest' }}</span>
            ...
            <div class="flex items-center">
                <div class="flex flex-col">
                    @auth
                    <span
                        class="text-sans text-[#C5C6CC] lg:text-sm xl:text-lg font-semibold">{{auth()->user()->first_name}}</span>
                    <a href="{{route('account')}}"><span
                            class="text-sans textwhite lg:text-base xl:text-xl font-bold">Account & Settings</span></a>
                    @else
                    <a href="{{route('login')}}"><span
                            class="text-sans text-[#C5C6CC] lg:text-sm xl:text-lg font-semibold">Login</span></a>
                    <a href="{{route('register')}}"><span
                            class="text-sans textwhite lg:text-base xl:text-xl font-bold">Register</span></a>
                    @endauth
                </div>
            </div>
    ...
    <div class="block lg:hidden p-4">
        ...
        <div class="flex items-center justify-between gap-4 w-full pt-5 relative">
            ...
            {{-- mobile cart button --}}
            <div class="flex items-center -mr-4 sm:-mr-6 lg:mr-0 border-0">
                @livewire('components.cart')

                <div x-data="{ mobileMenu: false }">
                    <div x-cloak x-transition x-show="mobileMenu"
                        class="absolute right-0 top-auto z-50 w-screen p-4 sm:max-w-xs">
                        <ul x-on:click.away="mobileMenu = false" class="p-6 space-y-4 shadow-xl rounded-xl">
                            @foreach ($this->collections as $collection)
                            <li>
                                <a class="text-sm font-medium"
                                    href="{{ route('collection.view', $collection->defaultUrl->slug) }}" wire:navigate>
        ...
        <div class="flex items-end justify-center gap-4 w-full -mt-6 pl-[15px]">
            @auth
            <a href="{{route('home')}}"
                class="{{ request()->routeIs('home') ? 'bg-[#1275EE]' : 'bg-[#1275EE4D]' }} rounded-[42px] px-6 py-1 font-white min-w-[126px] flex justify-center">Offers</a>
            <div>
                <a href="{{route('home')}}"><img class="w-[80px]" src="{{ asset('images/mobilelogo.png') }}" alt=""></a>
            </div>
            <a href="{{route('products.index')}}"
                class="rounded-[42px] px-6 py-1 font-white {{ request()->routeIs('products.index') ? 'bg-[#1275EE]' : 'bg-[#1275EE4D]' }}">Wholesale</a>
            @else
            <a href="{{route('login')}}"
                class="{{ request()->routeIs('login') ? 'bg-[#1275EE]' : 'bg-[#1275EE4D]' }} rounded-[42px] px-6 py-1 font-white min-w-[126px] flex justify-center">Login</a>
            <div>
                <a href="{{route('home')}}"><img class="w-[80px]" src="{{ asset('images/mobilelogo.png') }}" alt=""></a>
            </div>
            <a href="{{route('register')}}"
                class="rounded-[42px] px-6 py-1 font-white {{ request()->routeIs('products.index') ? 'bg-[#1275EE]' : 'bg-[#1275EE4D]' }}">Register</a>
            @endauth
        </div>
    ...
    <!-- Header menu - Mobile -->
    <div class="z-10 p-6 absolute w-full bg-themeblack min-h-screen transition-all duration-300 ease-out transform -translate-y-full"
        :class="{'translate-y-0': show}" x-show="show" @click.away="show = false">
        <div class="rounded-[15px] p-0 bg-white opacity-100 w-full">
            <div class="p-5">
                <h3 class="text-[24px] font-semibold text-[#000000]">Hi, {{ $user->first_name ?? 'Guest' }}!</h3>
            </div>
