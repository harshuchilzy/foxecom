<header class="w-full bg-themeblack text-white relative" x-data="{ show: false }">
    <div class="lg:flex items-center container mx-auto gap-8 hidden p-4">
        ...
        <div class="flex items-center justify-between gap-4 w-full">
            ...
            <div class="flex items-center gap-4 flex-wrap xl:flex-nowrap lg:w-[20%] xl:w-auto">
                ...
                <a href="{{route('products.index')}}" class="w-[150px] h-[40px] flex items-center justify-center gap-2 rounded-[60px] hover:bg-[#1275EE] {{ request()->routeIs('products.index') ? 'bg-[#1275EE]' : 'bg-[#1275EE4D]' }}">
                    <span>
    ...
    <div class="block lg:hidden p-4">
            ...
            <div class="flex items-center -mr-4 sm:-mr-6 lg:mr-0 border-0">
                ...
                <div x-data="{ mobileMenu: false }">
                    ...
                    <div x-cloak
                         ...
                         class="absolute right-0 top-auto z-50 w-screen p-4 sm:max-w-xs">
                        <ul x-on:click.away="mobileMenu = false"
                            class="p-6 space-y-4 shadow-xl rounded-xl">
                            @foreach ($this->collections as $collection)
                                <li>
                                    <a class="text-sm font-medium"
                                       href="{{ route('collection.view', $collection->defaultUrl->slug) }}"
        ...
        <div class="flex items-end justify-center gap-4 w-full -mt-6 pl-[15px]">
            <a href="{{route('home')}}" class="{{ request()->routeIs('home') ? 'bg-[#1275EE]' : 'bg-[#1275EE4D]' }} rounded-[42px] px-6 py-1 font-white min-w-[126px] flex justify-center">Offers</a>
            <div>
                <a href="{{route('home')}}"><img class="w-[80px]" src="{{ asset('images/mobilelogo.png') }}" alt=""></a>
            </div>
            <a href="{{route('products.index')}}" class="rounded-[42px] px-6 py-1 font-white {{ request()->routeIs('products.index') ? 'bg-[#1275EE]' : 'bg-[#1275EE4D]' }}">Wholesale</a>
        </div>
